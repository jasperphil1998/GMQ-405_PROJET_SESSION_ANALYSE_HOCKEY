# =============================================================================
# 02_autocorrelation.R — Autocorrelation spatiale (Moran, LISA, Getis-Ord)
# =============================================================================
# PROBLEME ADRESSE
# Le script principal montre visuellement que le hockey a une geographie, mais
# ne le DEMONTRE jamais statistiquement. L'autocorrelation spatiale repond a la
# question : la ressemblance entre regions voisines depasse-t-elle ce que le
# hasard produirait ?
#
# DEMARCHE
...
message("\n=== 02 — AUTOCORRELATION SPATIALE ===")

jrn <- journal_resultats("02_resultats_autocorrelation.txt")
jrn$ecrire("=====================================================")
jrn$ecrire(" ANALYSE SPATIALE — 02. AUTOCORRELATION SPATIALE")
jrn$ecrire("=====================================================")

# Reutilise l'objet prepare par 01 (evite de tout recalculer).
fichier_unites <- chemin_sortie("unites_normalisees.rds")
if (!file.exists(fichier_unites)) {
  stop("Lancer d'abord 01_normalisation.R (produit unites_normalisees.rds).",
       call. = FALSE)
}
...
sauver_tableau(sensibilite, "02_table_sensibilite_k.csv")
...
sauver_graphique(graph_moran, "02_graph_diagramme_moran.png",
                 largeur = 8, hauteur = 7)
...
sauver_carte(carte_lisa, "02_carte_lisa_grappes.png")
...
jrn$capturer(
  unites_gi %>%
    ...
    select(Code = postal, Unite = NomUnite, TauxPar100k, GiEtoile, ClasseGi) %>%
    head(12),
...
sauver_carte(carte_gi, "02_carte_getis_ord.png")
...
if (nrow(grille_analyse) > 20) {
  ...
  sauver_tableau(synthese, "02_table_synthese_echelles.csv")
  ...
  if (moran_grille_pts$p.value >= 0.05) {
    ...
    jrn$ecrire("   les cartes de points totaux du script principal, qui")
  ...
  sauver_carte(carte_lisa_grille, "02_carte_lisa_grille_hexagonale.png")
  ...
  sauver_carte(carte_pts_grille, "02_carte_grille_pts_moyens.png")
